...
namespace lm
{
	...
	public:
		...
		static lm::Vec3<T> getAngles(const Mat4<T>& mat4)
		{
			lm::Mat4<T> rotation = lm::Mat4<T>::getRotationMatrix(mat4);

			float x = lm::Vec3<T>::radiansToDegrees(std::atan2(rotation[1][2], rotation[2][2]));
			float y = lm::Vec3<T>::radiansToDegrees(std::atan2(-rotation["z0"], sqrt((rotation["z1"] * rotation["z1"]) + (rotation["z2"] * rotation["z2"])) ));
			float z = lm::Vec3<T>::radiansToDegrees(std::atan2(rotation[0][1], rotation[0][0]));

			return lm::Vec3<T>(x, y, z);
		}
